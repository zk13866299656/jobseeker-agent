...
### 7. 数据库压力（FAQ）
- **问：记忆会不会随对话变长而对数据库产生压力？**
- **答：不会。** `user_memory` 的唯一键 `(user_id, memory_type)` + 封闭 6 类 + upsert 覆盖，使每人记忆**上限 6 行**，不随对话长度增长；每句话仅 4~5 次走索引的毫秒级小查询。
- **真正随对话增长的是 `chat_message`**（会话历史，每句一行），本地单机单用户无感。
- **真正的瓶颈在 LLM 上下文窗口**（每轮重发历史+记忆，token 随历史增长），不在数据库——由第 5 点「上下文压缩」解决。
- 注意：第 2 点「记忆量增长→语义检索」的担忧，只在把唯一键升级为多值（每类可存多条）之后才成立；当前设计下记忆不会到「几十上百条」。
